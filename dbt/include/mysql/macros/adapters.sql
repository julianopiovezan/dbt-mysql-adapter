...
{% macro mysql__get_create_index_sql(relation, index_dict) -%}
  {%- set comma_separated_columns = index_dict.get('columns') | join(', ') -%}
  {%- set underscore_separated_columns = index_dict.get('columns') | join('_') -%}

  create {% if index_dict.get('unique', False) -%}
    unique
  {%- endif %} index
  {{ relation.table }}__{{ underscore_separated_columns }}_idx
  on {{ relation }}
  ({{ comma_separated_columns }});
{% endmacro %}

{% macro mysql__create_indexes(relation) -%}
  {%- set _indexes = config.get('indexes', default=[]) -%}

  {% for _index_dict in _indexes %}
    {% set create_index_sql = mysql__get_create_index_sql(relation, _index_dict) %}
    {% if create_index_sql %}
